...
test_table = 'orig_stay_booking_by_stay_date'
prod_table = 'f_stay_booking_summary'
file_temp_name = 'temp.xlsx'
col_map_old_to_new_name = 'column mapping existing to new (original specific)'
file_sql_random_stay_name = 'random property and stay population.sql'
file_s2t_name = 'orig_stay_booking_by_stay_date column and data type validation.xlsx'
...
df_col_map_old_to_new_with_mapping = df_col_map_old_to_new[df_col_map_old_to_new['new'] != '*no mapping*']
df_col_map_old_to_new_no_mapping = df_col_map_old_to_new[df_col_map_old_to_new['new'] == '*no mapping*']


col_map_old_to_new_list = df_col_map_old_to_new_with_mapping.to_numpy().tolist()
...
test_columns = [col for col in test_columns if col in dict_new_to_old]
prod_columns = [col for col in prod_columns if col in dict_old_to_new]
test_agg_column_set = {'orig_room_rate_local_amt'}
...
for elem in prop_and_stay:
    ...
    file_result = os.path.join(project_dir, f'{test_table} test environment vs PROD existing (original specific) ({prop_cd} stay_id {stay_id}).xlsx')
    ...
    labels_match = df_test.index.equals(df_prod.index) and df_test.columns.equals(df_prod.columns)
    if labels_match:
