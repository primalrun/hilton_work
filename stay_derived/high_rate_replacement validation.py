...
file_select_result = r'C:\Users\jwalker221\OneDrive - Hilton\Jira\Stay Derived\Honors Redemption and Rate Plan\preprod_validation\high_rate_replacement select extract.xlsx'
file_table_result = r'C:\Users\jwalker221\OneDrive - Hilton\Jira\Stay Derived\Honors Redemption and Rate Plan\preprod_validation\high_rate_replacement table extract.xlsx'
file_result = r'C:\Users\jwalker221\OneDrive - Hilton\Jira\Stay Derived\Honors Redemption and Rate Plan\preprod_validation\high_rate_replacement comparison result.xlsx'
...
compare_df['brand_code_variance'] = compare_df.apply(lambda x: is_same_text(x['brand_cd_select'], x['brand_cd_table']), axis=1)
compare_df['op_area_level_1_desc_variance'] = compare_df.apply(lambda x: is_same_text(x['op_area_level_1_desc_select'], x['op_area_level_1_desc_table']), axis=1)
compare_df['open_dt_variance'] = compare_df.apply(lambda x: is_same_text(x['open_dt_select'], x['open_dt_table']), axis=1)
compare_df['prop_replacement_ind_variance'] = compare_df.apply(lambda x: is_same_text(x['prop_replacement_ind_select'], x['prop_replacement_ind_table']), axis=1)

compare_df['prop_replacement_rate_var %'] = compare_df.apply(lambda x: calc_variance_pct(x['prop_replacement_rate_select'], x['prop_replacement_rate_table']), axis=1)
compare_df['rate_limit_var %'] = compare_df.apply(lambda x: calc_variance_pct(x['rate_limit_select'], x['rate_limit_table']), axis=1)
compare_df['region_replacement_rate_var %'] = compare_df.apply(lambda x: calc_variance_pct(x['region_replacement_rate_select'], x['region_replacement_rate_table']), axis=1)

append_df_to_existing_excel_workbook(compare_df, file_result, 'comparison')
sheet_format_dict['comparison'] = [{'#,##0': [10, 11, 12, 13, 14, 15], '0.0%': [22, 23, 24]}, 2, None]
