...
target_table = 'orig_stay_booking_by_stay_date'
file_sql_validation_name = 'orig_stay_booking_by_stay_date S2T validation (exclude rev columns) (stacked results) (dynamic).sql'
...
cursor.execute(sql_random_stay)
df_stay = cursor.fetch_dataframe()
...
for elem in prop_and_stay:
    ...
    sql = sql.replace('stay_id_variable', str(stay_id))
    cursor.execute(sql)
    ...
    non_variance_column = ['row_source', 'stay_id', 'stay_dt', 'orig_extract_last_update_dtm']
